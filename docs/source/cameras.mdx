...
| `SentechCamera`   | Omron Sentech GigE Vision cameras   |
...
`OpenCVCamera`, `RealSenseCamera`, and `SentechCamera` support auto-discovery. Run the command below to list available devices and their identifiers. Note that these identifiers may change after rebooting your computer or re-plugging the camera, depending on your operating system.

```bash
lerobot-find-cameras opencv # or realsense for Intel Realsense cameras
lerobot-find-cameras sentech --cti-path /path/to/Sentech_GenTL.cti
```
...
`SentechCamera` uses the GenTL producer (`*.cti`) installed with the Omron Sentech SDK. You can pass it with `--cti-path`, set `LEROBOT_SENTECH_CTI=/path/to/file.cti`, or use the standard `GENICAM_GENTL64_PATH`/`GENICAM_GENTL32_PATH` directory environment variables.
...
<hfoption id="Omron Sentech GigE Camera">
...
config = SentechCameraConfig(
    cti_path="/path/to/Sentech_GenTL.cti",
    ...
    color_mode=ColorMode.RGB,
    rotation=Cv2Rotation.NO_ROTATION,
    pixel_format="RGB8",  # Use "Mono8" or a Bayer8 format if that is what your camera exposes.
)
...
The same camera can be used from scripts:
...
lerobot-record \
  ...
  --robot.id=black \
  --robot.cameras='{front: {type: sentech, cti_path: "/path/to/Sentech_GenTL.cti", serial_number_or_name: "YOUR_CAMERA_SERIAL", width: 640, height: 480, fps: 30, pixel_format: "RGB8"}}' \
